...
class OBJECT_OT_check_scene_issues(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        with open(log_filename, "w") as log_file:
            ...
            for data_collection_name in ['meshes', 'materials', 'textures', 'images', 'objects', 'armatures', 'actions',
                                         'curves']:
                data_collection = getattr(bpy.data, data_collection_name,
                                          None)  # Use getattr and default None for compatibility
                if data_collection:
            ...
            if animation_errors:
                for obj_name in animation_errors:
                    log_file.write(
                        f"  - Animation Error: Object {obj_name} has animation data, but no Action is assigned.\n")
            else:
